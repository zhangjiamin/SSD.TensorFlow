...
class VGG16Backbone(object):
    ...
    def nearest_upsampling(self, data, scale):
        ...
        with tf.name_scope('nearest_upsampling'):
            if 'channels_first' == self._data_format:
            ...
            elif 'channels_last' == self._data_format:
    ...
    def forward(self, inputs, training=False):
        ...
        with tf.variable_scope('conv4_3_scale') as scope:
            ...
            l2_norm = tf.multiply(weight_scale, self.l2_normalize(inputs, name='norm'), name='rescale')
            feats_bottom_up[2] = l2_norm
        ...
        inputs = tf.layers.conv2d(inputs=inputs, filters=1024, kernel_size=1, strides=1, padding='same',
                            ...
                            name='fc7', reuse=None)
        # fc7
        feats_bottom_up[3]=inputs

        with tf.variable_scope('additional_layers') as scope:
            # forward ssd layers
            inputs = self.apply_ssd_conv_block(inputs, 256, 2, 'conv8')
            feats_bottom_up[4]=inputs
            # conv8
            inputs = self.apply_ssd_conv_block(inputs, 128, 2, 'conv9')
            feats_bottom_up[5]=inputs
            # conv9
            inputs = self.apply_ssd_conv_block(inputs, 128, 1, 'conv10', padding='valid')
            feats_bottom_up[6]=inputs
            # conv10
            inputs = self.apply_ssd_conv_block(inputs, 128, 1, 'conv11', padding='valid')
            feats_bottom_up[7]=inputs
            # conv11
            ...
            with tf.variable_scope('lateral_connections') as scope:
                for level in range(min_layer, max_layer + 1):
                    ...
                    channels = 256
                    feats_lateral[level] = self.apply_conv_block(feats_bottom_up[level], 1, channels, kernel_size=1, strides=1, name='lateral_{}'.format(level))
            ...
            with tf.variable_scope('post_hocs') as scope:
                for level in range(min_layer, max_layer + 1):
                    ...
                    feats[level] = self.apply_conv_block(feats[level], 1, channels, kernel_size=3, strides=1, name='post_hoc_{}'.format(level))
    ...
    def apply_ssd_conv_block(self, inputs, filters, strides, name, padding='same'):
        with tf.variable_scope(name):
            inputs = tf.layers.conv2d(inputs=inputs, filters=filters, kernel_size=1, strides=1, padding=padding,
                        ...
                        kernel_initializer=self._conv_initializer(),
                        bias_initializer=tf.zeros_initializer(),
                        name='{}_1'.format(name), reuse=None)
